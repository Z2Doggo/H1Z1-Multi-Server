...
#define PLATFORM_BUFFER_WRITE_TO_FILE(name)                                    \
  u32 name(char *file_path, void *buffer, u32 buffer_length)
...
#define PLATFORM_SOCKET_UDP_CREATE_AND_BIND(name)                              \
  Platform_Socket name(Platform_State *state, u16 port)
typedef PLATFORM_SOCKET_UDP_CREATE_AND_BIND(
    platform_socket_udp_create_and_bind);

#define PLATFORM_RECEIVE_FROM(name)                                            \
  i32 name(Platform_Socket sock, void *buffer, i32 length, u32 *from_ip,       \
           u16 *from_port)
typedef PLATFORM_RECEIVE_FROM(platform_receive_from);

#define PLATFORM_SEND_TO(name)                                                 \
  i32 name(Platform_Socket sock, void *buffer, i32 length, u32 to_ip,          \
           u16 to_port)
...
#define PLATFORM_ELAPSED_SECONDS(name)                                         \
  f32 name(Platform_State *state, u64 begin, u64 end)
