...
internal Fragment_Pool fragment_pool_create(u32 capacity, u32 packet_length,
  ...
  result.fragments = arena_push_array(arena, Fragment_Entry, capacity);
  result.buffer = arena_push_size(arena, packet_length * capacity);

  return result;
}

// TODO(rhett): make sure we're taking crc length into account in the fragment
// pool and we snip it off
internal void fragment_pool_insert(Fragment_Pool *pool, i32 sequence, u8 *data,
                                   u32 data_length, b32 is_fragment) {
  ...
  // printf("[X] fragment_pool_insert; %d (sequence=%d - base=%d) exceeds pool
  ...
  // printf("[X] fragment_pool_insert; Data length %d exceeds packet length
