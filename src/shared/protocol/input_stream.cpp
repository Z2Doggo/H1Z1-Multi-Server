...
internal void input_stream_channel_packet_data_parse(Input_Stream *stream,
  ...
  if (data[0] == 0x0 && data[1] == 0x19) {
    ...
    while (offset < data_length) {
      ...
      if (*stream->data_callback_ptr) {
        (*stream->data_callback_ptr)(server, session, data + offset,
                                     chunk_length);
      }
...
internal void input_stream_fragments_process(Input_Stream *stream, void *server,
                                             void *session) {
  ...
  if (head.data_length) {
    if (!head.is_fragment) {
      ...
      input_stream_channel_packet_data_parse(stream, head.data,
                                             head.data_length, server, session);
      fragment_pool_advance(stream->fragment_pool);
    } else {
      if (stream->fragment_pool->buffer_tail ==
          stream->fragment_pool->buffer_target_length) {
        // TODO(rhett): Might need to decrement this
        stream->processed_fragment_previous =
            stream->fragment_pool->fragments_count - 1;
        input_stream_channel_packet_data_parse(
            stream, head.data, stream->fragment_pool->buffer_target_length,
            server, session);
        fragment_pool_advance(stream->fragment_pool);
      }
    }
...
// TODO(rhett): Ack callback might want to use a session id or index instead of
// a pointer later on
internal void input_stream_write(Input_Stream *stream, u8 *data,
                                 u32 data_length, i32 sequence, b32 is_fragment,
  ...
  fragment_pool_insert(stream->fragment_pool, sequence, data, data_length,
                       is_fragment);
...
internal Input_Stream input_stream_init(Fragment_Pool *fragment_pool,
                                        u8 *rc4_key_decoded,
                                        u32 rc4_key_decoded_length,
                                        b32 use_encryption) {
  Input_Stream result = {0};

  crypt_rc4_initialize(&result.rc4_state, rc4_key_decoded,
                       rc4_key_decoded_length);
