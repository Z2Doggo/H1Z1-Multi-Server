void output_stream_write(Output_Stream *stream, u8 *data, u32 data_length,
                         b32 should_override_encryption, void *server,
                         void *session) {
  ...
  if (data_length <= stream->fragment_pool->packet_length) {
    ...
    if (*stream->data_callback_ptr) {
      (*stream->data_callback_ptr)(server, session, data, data_length,
                                   stream->sequence, FALSE);
    }
  } else {
    // TODO(rhett): still some cleanup to do
    // TODO(rhett): this will need to match output stream fragment length.
    // switch to an arena
    ASSERT((MAX_PACKET_LENGTH - DATA_HEADER_LENGTH) == (512 - 4) &&
           (MAX_PACKET_LENGTH - DATA_HEADER_LENGTH) ==
               stream->fragment_pool->packet_length);
    u8 temp_buffer[MAX_PACKET_LENGTH - DATA_HEADER_LENGTH] = {0};
    // TODO(rhett): maybe endian_write should return bytes written incase we
    // swap it out?
    endian_write_u32_big(temp_buffer, data_length);
    u32 initial_offset = sizeof(u32);

    for (u32 i = 0; i < data_length;) {
      u32 partial_data_length =
          MIN(data_length - i, stream->fragment_pool->packet_length);
      partial_data_length -= initial_offset;
      stream->sequence++;
      base_memory_copy(&temp_buffer[initial_offset], (void *)((uptr)data + i),
                       partial_data_length);

      // TODO(rhett):
      if (*stream->data_callback_ptr) {
        (*stream->data_callback_ptr)(server, session, temp_buffer,
                                     partial_data_length + initial_offset,
...
internal Output_Stream output_stream_init(Fragment_Pool *fragment_pool,
                                          u8 *rc4_key_decoded,
