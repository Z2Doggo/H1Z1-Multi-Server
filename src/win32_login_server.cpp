...
app_memory.backing_memory.data = reinterpret_cast<u8*>(VirtualAlloc(NULL, app_memory.backing_memory.size, MEM_COMMIT, PAGE_READWRITE));
	...
	while (is_running)
	{
		...
		bool is_code_locked = GetFileAttributesExA(MODULE_LOCK_FILE, GetFileExInfoStandard, &unused);
		FILETIME new_module_write_time = win32_get_last_write_time(MODULE_FILE);
		bool should_reload = !is_code_locked && CompareFileTime(&new_module_write_time, &app_code.module_last_write_time);
		if (should_reload)
		{
			printf("\n=======================================================================\n");
			printf("  Reloading...\n");
		...
		float elapsed_tick_seconds = platform_win_elapsed_seconds(previous_counter, platform_win_wall_clock());
		if (elapsed_tick_seconds < target_seconds_per_tick)
		{
			if (is_sleep_granular)
			{
				int sleep_ms = (int)(target_seconds_per_tick * 1000.0f) - (int)(elapsed_tick_seconds * 1000.0f) - 1;
