...
internal bool platform_win_folder_create(const char* folder_path)
{
    if (!CreateDirectoryA(folder_path, NULL))
    {
        ...
        {
            std::cout << "[!] CreateDirectory error on \"" << folder_path << "\" - Error: " << last_error << std::endl;
            return false;
        }
    }
...
{
    HANDLE file_handle = CreateFileA(file_path, FILE_APPEND_DATA, 0, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
{
    HANDLE file_handle = CreateFileA(file_path, GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
internal int platform_win_receive_from(SOCKET socket, char* buffer, int size, unsigned int* from_ip, unsigned short* from_port)
{
    sockaddr_in from_address = {0};
    int from_address_size = sizeof(from_address);

    int result = recvfrom(socket, buffer, size, 0, reinterpret_cast<sockaddr*>(&from_address), &from_address_size);
...
int platform_win_send_to(SOCKET socket, const char* buffer, int size, unsigned int to_ip, unsigned short to_port)
{
    ...
    int result = sendto(socket, buffer, size, 0, reinterpret_cast<sockaddr*>(&to_address), sizeof(to_address));
