...
#if defined(BASE_SLOW)
#define ASSERT(expr)                                                           \
  if (!(expr)) {                                                               \
    ABORT;                                                                     \
  }
// TODO(rhett): Probably don't need to wrap msg in parens or logical NOTs,
// right?
...
#define STATIC_ASSERT_INTERNAL_1(cond, line)                                   \
  typedef char static_assert_line_##line[(!!(cond)) * 2 - 1]
#define STATIC_ASSERT_INTERNAL_2(cond, line)                                   \
  STATIC_ASSERT_INTERNAL_1(cond, line)
...
#define PARAMS_BIND(name, defaults, ...)                                       \
  name((name##_params){defaults, ._ = 0, __VA_ARGS__})

// #define foo(...) PARAMS_BIND(foo, PASS_PARAMS((.a=1, .b=2, .c=3, .d=4)),
// __VA_ARGS__) PARAMS_DECLARE(foo, int a; int b; int c; int d, void)
//{
...
// TODO(rhett): Use length instead here? Or will that be more confusing since
// they're equal in this context?
...
#define ARENA_DEFAULT_PARAMS                                                   \
  PASS_PARAMS((.should_clear = TRUE, .alignment = ARENA_ALIGN_DEFAULT))
...
#define arena_push_size(...)                                                   \
  PARAMS_BIND(arena_push_size, ARENA_DEFAULT_PARAMS, __VA_ARGS__)
PARAMS_DECLARE(void *, arena_push_size, Arena *arena; isize size;
  ...
  uptr aligned_ptr = base_align_forward(
      (uptr)params.arena->buffer + params.arena->tail_offset, params.alignment);
  isize padding = 0;
  padding =
      aligned_ptr - ((uptr)params.arena->buffer + params.arena->tail_offset);

  // TODO(rhett): Assert for now. we should be able to recover from this later I
  // think
  ASSERT_MSG((params.size + padding) < params.arena->capacity,
             "Allocation excedes arena capacity");
...
#define arena_push_copy(...)                                                   \
  PARAMS_BIND(arena_push_copy, ARENA_DEFAULT_PARAMS, __VA_ARGS__)
PARAMS_DECLARE(void *, arena_push_copy, Arena *arena; void *source;
               isize source_size; b32 should_clear; isize alignment) {
  ASSERT(params.arena);
  ASSERT(params.source);
  ASSERT(params.source_size);

  void *result = arena_push_size(params.arena, params.source_size,
                                 params.should_clear, params.alignment);
...
PARAMS_DECLARE(void *, arena_push_copy_zero_terminate, Arena *arena;
               void *source; isize source_size; b32 should_clear;
               isize alignment) {
  ASSERT(params.arena);
  ASSERT(params.source);
  ASSERT(params.source_size);

  void *result = arena_push_size(params.arena, params.source_size + 1,
                                 params.should_clear, params.alignment);
  base_memory_copy(result, params.source, params.source_size);
  return result;
}
...
#define arena_push_copy_ztstring_as_string(...)                                \
  PARAMS_BIND(arena_push_copy_ztstring_as_string, ARENA_DEFAULT_PARAMS,        \
              __VA_ARGS__)
PARAMS_DECLARE(Buffer, arena_push_copy_ztstring_as_string, Arena *arena;
               void *source; b32 should_clear; isize alignment) {
  ASSERT(params.arena);
  ASSERT(params.source);
  ...
  Buffer result = {
      ...
      .data = arena_push_size(params.arena, result.size + 1,
                              params.should_clear, params.alignment),
  };
...
PARAMS_DECLARE(void *, arena_bootstrap_push_size, void *buffer; isize capacity;
               char *name; isize struct_size; isize offset_to_arena;
               b32 should_clear; isize alignment) {
  ...
  void *bootstrap_struct = arena_push_size(
      &bootstrap, params.struct_size, params.should_clear, params.alignment);
...
#endif // BASE_USE_ARENA

//====================================================================================================
// String - String related functions which require higher layers such as Memory
// Arena
//====================================================================================================

#if defined(BASE_USE_STRING)
...
Substring_List string_ztstring_copy_and_split(char *source, char delim,
                                              isize max_substrings,
                                              Arena *arena) {
    ...
    if (current_char == '\"') {
      ...
      if (is_within_arg) {
        ...
        result.substrings[result.substrings_count].data =
            arena_push_copy_zero_terminate(
