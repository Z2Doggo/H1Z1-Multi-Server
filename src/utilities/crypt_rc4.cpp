#if 0
static void
	crypt_rc4_generate_keystream()
{
	u8 array[256] = {0};

	for (u32 i = 0; i < sizeof(array); i++)
	{
		array[i] = i;
	}

	u8 buffer[KB(4)] = {0};
	u32 offset = 0;
	offset += sprintf(buffer+offset, "u8 keystream[%zu] = {\n", sizeof(array));
	for (u32 i = 0; i < sizeof(array); i++)
	{
		offset += sprintf(buffer+offset, "0x%02x, ", array[i]);
		if (!((i + 1) % 8))
		{
			offset += sprintf(buffer+offset, "\n");
		}
	}
	offset += sprintf(buffer+offset, "};");

	win32_write_buffer_to_file("rc4_keystream.c", buffer, offset);
}
#endif

// TODO(rhett): hardcoded length of 256 right now
typedef struct Rc4_State Rc4_State;
struct Rc4_State {
  u8 keystream[256];
  u32 index_0;
  u32 index_1;
};

static void crypt_rc4_initialize(Rc4_State *state, u8 *key, u32 key_length) {
  u8 keystream_initial[256] = {
      };

  base_memory_copy(state->keystream, keystream_initial, 256);

  // NOTE(rhett): Yes, use a temp variable for the swap.
  u8 swap;

  u32 i, j;
  for (i = j = 0; i < 256; i++) {
    j = (j + state->keystream[i] + key[i % key_length]) % 256;

    swap = state->keystream[i];
    state->keystream[i] = state->keystream[j];
    state->keystream[j] = swap;
  }
}

static void crypt_rc4_transform(Rc4_State *state, u8 *data, u32 data_length) {
  // NOTE(rhett): Yes, use a temp variable for the swap.
  u8 swap;

  for (u32 k = 0; k < data_length; k++) {
    state->index_0 = (state->index_0 + 1) % 256;
    state->index_1 = (state->index_1 + state->keystream[state->index_0]) % 256;

    swap = state->keystream[state->index_0];
    state->keystream[state->index_0] = state->keystream[state->index_1];
    state->keystream[state->index_1] = swap;

    data[k] ^= state->keystream[(state->keystream[state->index_0] +
                                 state->keystream[state->index_1]) %
                                256];
  }
}
